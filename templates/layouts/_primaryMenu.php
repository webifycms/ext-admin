...
>
    <ul class="grid grid-cols-3 xl:grid-cols-6 gap-4 xl:gap-8">
        <?php foreach (view()->params['primaryMenuItems'] as $item) { ?>
            <li>
                <a href="<?php echo url($item['route']); ?>" class="block text-center" role="menuitem">
                    <svg class="block mx-auto w-6 h-6 xl:w-8 xl:h-8" width="32" height="32" fill="currentColor">
                        <use xlink:href="<?php echo $asset->baseUrl . '/icons/icons.svg#' . $item['icon']; ?>"/>
                    </svg>
                    <span class="block mt-2 mx-auto"><?php echo $item['label']; ?></span>
                </a>
            </li>
        <?php } ?>
    </ul>
